...
Read [docs/COURSE_SPEC.md](docs/COURSE_SPEC.md) in full. It is the authoritative educational specification and must remain intact. This document supplies the software architecture, implementation sequence, and acceptance criteria.
...
| `/onboarding` | Three short question groups from the course spec, editable time budget, timezone, explanation language, optional exam date, saved draft |
| `/diagnostic` | Short resumable rounds; objective and productive evidence; explicit skipped/unassessed audio skills |
...
Create a coverage matrix mapping every educational requirement in `COURSE_SPEC.md` to module/lesson IDs, practice, reference support, and status. Completion counts never substitute for coverage or language quality.
...
M2 expands the bridge to cover all eight bridge areas, then publishes an initial full C1 sequence of at least 12 modules with four substantial lessons each. Cover applications/interviews, workplace organization, projects, customers/complaints, sales/purchasing, quality/processes, conflict, leadership, presentations, digitalization/change, sustainability, and professional development. Map grammar and communicative skills across these topics. Expand the sequence where the coverage matrix identifies gaps.
...
The article toolkit must include all four cases, singular/plural, indefinite/possessive/negative/zero articles, contractions, gender patterns and exceptions, weak nouns, plural patterns, adjective agreement, compounds, and a lookup decision guide. Test common overgeneralizations explicitly. Store rule scope and exception links.
...
| Learner profile | User ID, goals, language preferences, time budget, study days, timezone, optional exam date, diagnostic state |
...
| Error occurrence | Attempt/evaluation reference, category, original/corrected text, explanation, repair/retest links |
...
| Exam definition/attempt | Official-source/version metadata, rule configuration, mock version, mode, deadlines, saved responses, integrity flags, rubric and results |
...
Build context from the current published lesson, relevant reference entries, learner goals, targeted errors, and a bounded recent history. Use PostgreSQL search and stable references initially; a vector database is not needed for the first release. The database is the learning record even if a provider offers conversation state.
...
Implement a versioned exam definition from current official telc/BAMF materials. Use the checked overview in `COURSE_SPEC.md` only as a starting point. Verify detailed subparts, response formats, scoring weights, pass conditions, audio rules, speaking sequence, preparation rules, and the relevant delivery mode before publishing a definition.
...
Use server-authoritative start times, section deadlines, attempt state, and finalization. The browser timer is a display. Reload, duplicate tabs, clock changes, and delayed background jobs cannot extend an exam. Save drafts with sequence/version checks; retrying submission is idempotent. Grade only server-accepted responses within the permitted window.
...
Apply verified rubric arithmetic in deterministic code. Open-response ratings remain AI coaching estimates with criterion evidence. Readiness requires at least two distinct complete valid mocks meeting the verified criteria; show each score and distance from each relevant boundary. Define a coaching margin separately and label it. Missing speaking or listening evidence keeps overall readiness incomplete. Do not invent a statistical probability of passing.
...
| M2 — Complete curriculum foundation | Remaining bridge and full C1 content sequence, coverage matrix, complete reference/lexical systems, search, diagrams, multiple exercise types, immutable content import | Every course requirement has a mapped location; all published dependencies/links resolve; no placeholder lessons or unexplained answer keys; audio-dependent availability is truthful |
...
1. Read both specifications, repository instructions, and existing code. Identify the next incomplete milestone from actual behavior and the implementation record.
