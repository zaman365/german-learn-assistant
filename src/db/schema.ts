...
export const attempts = pgTable(
  ...
  (t) => [
    uniqueIndex("attempt_idempotency_idx").on(t.userId, t.idempotencyKey),
    index("attempt_owner_skill_idx").on(t.userId, t.skill),
  ],
);
export type EvaluationData = {
  ...
  explanation: string;
  correction?: string;
  criteria?: { criterion: string; result: string; evidence: string }[];
  limitations?: string[];
};
export const evaluations = pgTable(
  "evaluations",
  {
    ...
    data: jsonb("data").$type<EvaluationData>().notNull(),
...
export const exposures = pgTable(
  ...
  (t) => [
    uniqueIndex("exposure_key_idx").on(
      t.userId,
      t.exerciseId,
      t.contentVersion,
      t.attemptKey,
...
export const reviews = pgTable(
  ...
  (t) => [
    uniqueIndex("review_target_idx").on(t.userId, t.targetId, t.mode),
    index("review_due_idx").on(t.userId, t.dueDate),
  ],
);
export const errorPatterns = pgTable(
  ...
  (t) => [
    uniqueIndex("error_pattern_idx").on(t.userId, t.skill, t.tag),
    index("error_owner_idx").on(t.userId),
  ],
);
export const placement = pgTable(
  "placement",
  {
    ...
    waivedObjectives: jsonb("waived_objectives").$type<string[]>().notNull(),
    policy: text("policy").notNull(),
    updatedAt: timestamp("updated_at", { withTimezone: true })
      .notNull()
      .defaultNow(),
